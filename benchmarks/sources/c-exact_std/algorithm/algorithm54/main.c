// Generated by cir2c - CIR (ClangIR) lowered to verifier-friendly C.
// Verifier should support: pointers, array element address (&a[i]),
//   pointer difference (p - q), pointer reinterpretation casts, structs,
//   memcpy, heap allocation (malloc/free), integer overflow builtins.

extern void abort(void);
// Struct definitions (auto-parsed)
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ { int* _M_current; };
struct __gnu_cxx____ops___Iter_less_iter { unsigned char __field0; };
struct std___UninitDestroyGuard_int____void_ { int* _M_first; int** _M_cur; };
struct std___Vector_base_int__std__allocator_int_____Vector_impl_data { int* _M_start; int* _M_finish; int* _M_end_of_storage; };
struct std____new_allocator_int_ { unsigned char __field0; };
struct std__allocator_int_ { unsigned char __field0; };
struct std__random_access_iterator_tag { unsigned char __field0; };
struct std___Vector_base_int__std__allocator_int_____Vector_impl { struct std___Vector_base_int__std__allocator_int_____Vector_impl_data __field0; };
struct std___Vector_base_int__std__allocator_int__ { struct std___Vector_base_int__std__allocator_int_____Vector_impl _M_impl; };
struct std__vector_int__std__allocator_int__ { struct std___Vector_base_int__std__allocator_int__ __field0; };

int __const_main_second[5] = {10, 20, 30, 40, 50};
int __const_main_first[5] = {5, 10, 15, 20, 25};
char _str[10] = "v[0] == 5";
char _str_1[111] = "/home/levente/Documents/University/cir/cir-demo/benchmarks/sources/cpp-baseline/algorithm/algorithm54/main.cpp";
char __PRETTY_FUNCTION___main[11] = "int main()";
char _str_2[11] = "v[1] == 15";
char _str_3[11] = "v[2] == 25";
char _str_4[49] = "cannot create std::vector larger than max_size()";
char _str_5[91] = "/usr/lib64/gcc/x86_64-pc-linux-gnu/15.2.1/../../../../include/c++/15.2.1/bits/stl_vector.h";
char __PRETTY_FUNCTION____ZNSt6vectorIiSaIiEEixEm[92] = "reference std::vector<int>::operator[](size_type) [_Tp = int, _Alloc = std::allocator<int>]";
char _str_6[19] = "__n < this->size()";
void std__allocator_int___allocator_2(struct std__allocator_int_* p0);
void std__vector_int__std__allocator_int_____vector(struct std__vector_int__std__allocator_int__* p0, unsigned long p1, struct std__allocator_int_* p2);
void std__allocator_int____allocator(struct std__allocator_int_* p0);
void __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________normal_iterator_2(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p0);
_Bool bool___gnu_cxx____ops___Iter_less_iter__operator___int___int___int___int___const(struct __gnu_cxx____ops___Iter_less_iter* p0, int* p1, int* p2);
int* __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator____const(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p0);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator__(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p0);
void __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________normal_iterator(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p0, int** p1);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator__long__const(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p0, long p1);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____niter_wrap___gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______int__(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ p0, int* p1);
long std__iterator_traits_int____difference_type_std____distance_int__(int* p0, int* p1, struct std__random_access_iterator_tag p2);
long std__iterator_traits_int____difference_type_std__distance_int__(int* p0, int* p1);
extern void* memmove(void* p0, void* p1, unsigned long p2);
void void_std____assign_one_false__int___int__(int** p0, int** p1);
int* int__std____copy_move_a2_false__int___int___int__(int* p0, int* p1, int* p2);
int* int__std____copy_move_a1_false__int___int__(int* p0, int* p1, int* p2);
int* int__std____niter_base_int__(int* p0);
int** __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______base___const(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p0);
int* int__std____niter_base_int___std__vector_int__std__allocator_int_____(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ p0);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____copy_move_a_false__int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(int* p0, int* p1, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ p2);
int* int__std____miter_base_int__(int* p0);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std__copy_int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(int* p0, int* p1, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ p2);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____set_difference_int___int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________gnu_cxx____ops___Iter_less_iter_(int* p0, int* p1, int* p2, int* p3, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ p4, struct __gnu_cxx____ops___Iter_less_iter p5);
struct __gnu_cxx____ops___Iter_less_iter __gnu_cxx____ops____iter_less_iter();
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std__set_difference_int___int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(int* p0, int* p1, int* p2, int* p3, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ p4);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ std__vector_int__std__allocator_int_____begin(struct std__vector_int__std__allocator_int__* p0);
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator_(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p0, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* p1);
unsigned long std__vector_int__std__allocator_int_____size___const(struct std__vector_int__std__allocator_int__* p0);
extern void std____glibcxx_assert_fail(char* p0, int p1, char* p2, char* p3);
int* std__vector_int__std__allocator_int_____operator__(struct std__vector_int__std__allocator_int__* p0, unsigned long p1);
extern void reach_error();
extern void __assert_fail(char* p0, char* p1, unsigned int p2, char* p3);
void std__vector_int__std__allocator_int______vector(struct std__vector_int__std__allocator_int__* p0);
int main();
void std____new_allocator_int_____new_allocator_2(struct std____new_allocator_int_* p0);
unsigned long* unsigned_long_const__std__min_unsigned_long_(unsigned long* p0, unsigned long* p1);
unsigned long std__vector_int__std__allocator_int______S_max_size(struct std__allocator_int_* p0);
void std__allocator_int___allocator(struct std__allocator_int_* p0, struct std__allocator_int_* p1);
extern void std____throw_length_error(char* p0);
unsigned long std__vector_int__std__allocator_int______S_check_init_len(unsigned long p0, struct std__allocator_int_* p1);
void std___Vector_base_int__std__allocator_int______Vector_impl___Vector_impl(struct std___Vector_base_int__std__allocator_int_____Vector_impl* p0, struct std__allocator_int_* p1);
_Bool std____is_constant_evaluated();
extern void std____throw_bad_array_new_length();
extern void *malloc(unsigned long);
extern void free(void*);
void* operator_new(unsigned long p0) { return malloc(p0); }
unsigned long std____new_allocator_int____M_max_size___const(struct std____new_allocator_int_* p0);
extern void std____throw_bad_alloc();
void* operator_new_2(unsigned long p0, unsigned long p1) { return malloc(p0); }
int* std____new_allocator_int___allocate(struct std____new_allocator_int_* p0, unsigned long p1, void* p2);
int* std__allocator_int___allocate(struct std__allocator_int_* p0, unsigned long p1);
int* std__allocator_traits_std__allocator_int_____allocate(struct std__allocator_int_* p0, unsigned long p1);
int* std___Vector_base_int__std__allocator_int______M_allocate(struct std___Vector_base_int__std__allocator_int__* p0, unsigned long p1);
void std___Vector_base_int__std__allocator_int______M_create_storage(struct std___Vector_base_int__std__allocator_int__* p0, unsigned long p1);
void std___Vector_base_int__std__allocator_int______Vector_impl____Vector_impl(struct std___Vector_base_int__std__allocator_int_____Vector_impl* p0);
void std___Vector_base_int__std__allocator_int______Vector_base(struct std___Vector_base_int__std__allocator_int__* p0, unsigned long p1, struct std__allocator_int_* p2);
_Bool std__is_constant_evaluated();
void std___UninitDestroyGuard_int___void____UninitDestroyGuard(struct std___UninitDestroyGuard_int____void_* p0, int** p1);
int* _ZSt12construct_atIiJEQaant20is_unbounded_array_vIT_ErqXgsnwcvPvLi0E_S0_pispclsr3stdE7declvalIT0_EEEEEPS0_S3_DpOS2_(int* p0);
void void_std___Construct_int_(int* p0);
void std___UninitDestroyGuard_int___void___release(struct std___UninitDestroyGuard_int____void_* p0);
void std___UninitDestroyGuard_int___void_____UninitDestroyGuard(struct std___UninitDestroyGuard_int____void_* p0);
int* int__std____uninitialized_default_n_1_false_____uninit_default_n_int___unsigned_long_(int* p0, unsigned long p1);
void void_std____fill_a1_int___int_(int* p0, int* p1, int* p2);
void void_std____fill_a_int___int_(int* p0, int* p1, int* p2);
int* int__std____fill_n_a_int___unsigned_long__int_(int* p0, unsigned long p1, int* p2, struct std__random_access_iterator_tag p3);
unsigned long std____size_to_integer(unsigned long p0);
struct std__random_access_iterator_tag std__iterator_traits_int____iterator_category_std____iterator_category_int__(int** p0);
int* int__std__fill_n_int___unsigned_long__int_(int* p0, unsigned long p1, int* p2);
int* int__std____uninitialized_default_n_1_true_____uninit_default_n_int___unsigned_long_(int* p0, unsigned long p1);
int* int__std____uninitialized_default_n_int___unsigned_long_(int* p0, unsigned long p1);
int* int__std____uninitialized_default_n_a_int___unsigned_long__int_(int* p0, unsigned long p1, struct std__allocator_int_* p2);
struct std__allocator_int_* std___Vector_base_int__std__allocator_int______M_get_Tp_allocator(struct std___Vector_base_int__std__allocator_int__* p0);
void std__vector_int__std__allocator_int______M_default_initialize(struct std__vector_int__std__allocator_int__* p0, unsigned long p1);
void operator_delete(void* p0) { free(p0); }
void operator_delete_3(void* p0, unsigned long p1, unsigned long p2) { free(p0); }
void operator_delete_2(void* p0, unsigned long p1) { free(p0); }
void std____new_allocator_int___deallocate(struct std____new_allocator_int_* p0, int* p1, unsigned long p2);
void std__allocator_int___deallocate(struct std__allocator_int_* p0, int* p1, unsigned long p2);
void std__allocator_traits_std__allocator_int_____deallocate(struct std__allocator_int_* p0, int* p1, unsigned long p2);
void std___Vector_base_int__std__allocator_int______M_deallocate(struct std___Vector_base_int__std__allocator_int__* p0, int* p1, unsigned long p2);
void std___Vector_base_int__std__allocator_int_______Vector_base(struct std___Vector_base_int__std__allocator_int__* p0);
void std____new_allocator_int_____new_allocator(struct std____new_allocator_int_* p0, struct std____new_allocator_int_* p1);
void std___Vector_base_int__std__allocator_int______Vector_impl_data___Vector_impl_data(struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* p0);
void void_std__destroy_at_int_(int* p0);
void void_std___Destroy_int__(int* p0, int* p1);
void void_std___Destroy_int___int_(int* p0, int* p1, struct std__allocator_int_* p2);

extern void *memcpy(void*, const void*, unsigned long);
// function: _ZNSaIiEC2Ev
void std__allocator_int___allocator_2(struct std__allocator_int_* v0) {
bb1: ;
  struct std__allocator_int_* this2;
  this2 = v0;
  struct std__allocator_int_* t3 = this2;
  struct std____new_allocator_int_* base4 = (struct std____new_allocator_int_*)((char *)t3 + 0);
  std____new_allocator_int_____new_allocator_2(base4);
  return;
}

// function: _ZNSt6vectorIiSaIiEEC2EmRKS0_
void std__vector_int__std__allocator_int_____vector(struct std__vector_int__std__allocator_int__* v5, unsigned long v6, struct std__allocator_int_* v7) {
bb8: ;
  struct std__vector_int__std__allocator_int__* this9;
  unsigned long __n10;
  struct std__allocator_int_* __a11;
  this9 = v5;
  __n10 = v6;
  __a11 = v7;
  struct std__vector_int__std__allocator_int__* t12 = this9;
  struct std___Vector_base_int__std__allocator_int__* base13 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t12 + 0);
  unsigned long t14 = __n10;
  struct std__allocator_int_* t15 = __a11;
  unsigned long r16 = std__vector_int__std__allocator_int______S_check_init_len(t14, t15);
  struct std__allocator_int_* t17 = __a11;
  std___Vector_base_int__std__allocator_int______Vector_base(base13, r16, t17);
    unsigned long t18 = __n10;
    std__vector_int__std__allocator_int______M_default_initialize(t12, t18);
  return;
}

// function: _ZNSaIiED2Ev
void std__allocator_int____allocator(struct std__allocator_int_* v19) {
bb20: ;
  struct std__allocator_int_* this21;
  this21 = v19;
  struct std__allocator_int_* t22 = this21;
  return;
}

// function: _ZN9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEC2Ev
void __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________normal_iterator_2(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v23) {
bb24: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* this25;
  this25 = v23;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t26 = this25;
  int* c27 = ((void*)0);
  t26->_M_current = c27;
  return;
}

// function: _ZNK9__gnu_cxx5__ops15_Iter_less_iterclIPiS3_EEbT_T0_
_Bool bool___gnu_cxx____ops___Iter_less_iter__operator___int___int___int___int___const(struct __gnu_cxx____ops___Iter_less_iter* v28, int* v29, int* v30) {
bb31: ;
  struct __gnu_cxx____ops___Iter_less_iter* this32;
  int* __it133;
  int* __it234;
  _Bool __retval35;
  this32 = v28;
  __it133 = v29;
  __it234 = v30;
  struct __gnu_cxx____ops___Iter_less_iter* t36 = this32;
  int* t37 = __it133;
  int t38 = *t37;
  int* t39 = __it234;
  int t40 = *t39;
  _Bool c41 = ((t38 < t40)) ? 1 : 0;
  __retval35 = c41;
  _Bool t42 = __retval35;
  return t42;
}

// function: _ZNK9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEdeEv
int* __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator____const(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v43) {
bb44: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* this45;
  int* __retval46;
  this45 = v43;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t47 = this45;
  int* t48 = t47->_M_current;
  __retval46 = t48;
  int* t49 = __retval46;
  return t49;
}

// function: _ZN9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEppEv
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator__(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v50) {
bb51: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* this52;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* __retval53;
  this52 = v50;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t54 = this52;
  int* t55 = t54->_M_current;
  int c56 = 1;
  int* ptr57 = &(t55)[c56];
  t54->_M_current = ptr57;
  __retval53 = t54;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t58 = __retval53;
  return t58;
}

// function: _ZN9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEC2ERKS1_
void __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________normal_iterator(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v59, int** v60) {
bb61: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* this62;
  int** __i63;
  this62 = v59;
  __i63 = v60;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t64 = this62;
  int** t65 = __i63;
  int* t66 = *t65;
  t64->_M_current = t66;
  return;
}

// function: _ZNK9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEplEl
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator__long__const(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v67, long v68) {
bb69: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* this70;
  long __n71;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __retval72;
  int* ref_tmp073;
  this70 = v67;
  __n71 = v68;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t74 = this70;
  int* t75 = t74->_M_current;
  long t76 = __n71;
  int* ptr77 = &(t75)[t76];
  ref_tmp073 = ptr77;
  __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________normal_iterator(&__retval72, &ref_tmp073);
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t78 = __retval72;
  return t78;
}

// function: _ZSt12__niter_wrapIN9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEES2_ET_S7_T0_
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____niter_wrap___gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______int__(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ v79, int* v80) {
bb81: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __from82;
  int* __res83;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __retval84;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ agg_tmp085;
  __from82 = v79;
  __res83 = v80;
  int* t86 = __res83;
  int* r87 = int__std____niter_base_int__(t86);
  agg_tmp085 = __from82; // copy
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t88 = agg_tmp085;
  int* r89 = int__std____niter_base_int___std__vector_int__std__allocator_int_____(t88);
  long diff90 = r87 - r89;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ r91 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator__long__const(&__from82, diff90);
  __retval84 = r91;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t92 = __retval84;
  return t92;
}

// function: _ZSt10__distanceIPiENSt15iterator_traitsIT_E15difference_typeES2_S2_St26random_access_iterator_tag
long std__iterator_traits_int____difference_type_std____distance_int__(int* v93, int* v94, struct std__random_access_iterator_tag v95) {
bb96: ;
  int* __first97;
  int* __last98;
  struct std__random_access_iterator_tag unnamed99;
  long __retval100;
  __first97 = v93;
  __last98 = v94;
  unnamed99 = v95;
  int* t101 = __last98;
  int* t102 = __first97;
  long diff103 = t101 - t102;
  __retval100 = diff103;
  long t104 = __retval100;
  return t104;
}

// function: _ZSt8distanceIPiENSt15iterator_traitsIT_E15difference_typeES2_S2_
long std__iterator_traits_int____difference_type_std__distance_int__(int* v105, int* v106) {
bb107: ;
  int* __first108;
  int* __last109;
  long __retval110;
  struct std__random_access_iterator_tag agg_tmp0111;
  __first108 = v105;
  __last109 = v106;
  int* t112 = __first108;
  int* t113 = __last109;
  struct std__random_access_iterator_tag r114 = std__iterator_traits_int____iterator_category_std____iterator_category_int__(&__first108);
  agg_tmp0111 = r114;
  struct std__random_access_iterator_tag t115 = agg_tmp0111;
  long r116 = std__iterator_traits_int____difference_type_std____distance_int__(t112, t113, t115);
  __retval110 = r116;
  long t117 = __retval110;
  return t117;
}

// function: _ZSt12__assign_oneILb0EPiS0_EvRT0_RT1_
void void_std____assign_one_false__int___int__(int** v118, int** v119) {
bb120: ;
  int** __out121;
  int** __in122;
  __out121 = v118;
  __in122 = v119;
    int** t123 = __in122;
    int* t124 = *t123;
    int t125 = *t124;
    int** t126 = __out121;
    int* t127 = *t126;
    *t127 = t125;
  return;
}

// function: _ZSt14__copy_move_a2ILb0EPiS0_S0_ET2_T0_T1_S1_
int* int__std____copy_move_a2_false__int___int___int__(int* v128, int* v129, int* v130) {
bb131: ;
  int* __first132;
  int* __last133;
  int* __result134;
  int* __retval135;
  __first132 = v128;
  __last133 = v129;
  __result134 = v130;
      _Bool r136 = std____is_constant_evaluated();
      if (r136) {
      } else {
          long __n137;
          int* t138 = __first132;
          int* t139 = __last133;
          long r140 = std__iterator_traits_int____difference_type_std__distance_int__(t138, t139);
          __n137 = r140;
            long t141 = __n137;
            long c142 = 1;
            _Bool c143 = ((t141 > c142)) ? 1 : 0;
            if (c143) {
              int* t144 = __result134;
              void* cast145 = (void*)t144;
              int* t146 = __first132;
              void* cast147 = (void*)t146;
              long t148 = __n137;
              unsigned long cast149 = (unsigned long)t148;
              unsigned long c150 = 4;
              unsigned long b151 = cast149 * c150;
              void* r152 = memmove(cast145, cast147, b151);
              long t153 = __n137;
              int* t154 = __result134;
              int* ptr155 = &(t154)[t153];
              __result134 = ptr155;
            } else {
                long t156 = __n137;
                long c157 = 1;
                _Bool c158 = ((t156 == c157)) ? 1 : 0;
                if (c158) {
                  void_std____assign_one_false__int___int__(&__result134, &__first132);
                  int* t159 = __result134;
                  int c160 = 1;
                  int* ptr161 = &(t159)[c160];
                  __result134 = ptr161;
                }
            }
          int* t162 = __result134;
          __retval135 = t162;
          int* t163 = __retval135;
          return t163;
      }
    while (1) {
      int* t165 = __first132;
      int* t166 = __last133;
      _Bool c167 = ((t165 != t166)) ? 1 : 0;
      if (!c167) break;
      void_std____assign_one_false__int___int__(&__result134, &__first132);
    for_step164: ;
      int* t168 = __result134;
      int c169 = 1;
      int* ptr170 = &(t168)[c169];
      __result134 = ptr170;
      int* t171 = __first132;
      int c172 = 1;
      int* ptr173 = &(t171)[c172];
      __first132 = ptr173;
    }
  int* t174 = __result134;
  __retval135 = t174;
  int* t175 = __retval135;
  return t175;
}

// function: _ZSt14__copy_move_a1ILb0EPiS0_ET1_T0_S2_S1_
int* int__std____copy_move_a1_false__int___int__(int* v176, int* v177, int* v178) {
bb179: ;
  int* __first180;
  int* __last181;
  int* __result182;
  int* __retval183;
  __first180 = v176;
  __last181 = v177;
  __result182 = v178;
  int* t184 = __first180;
  int* t185 = __last181;
  int* t186 = __result182;
  int* r187 = int__std____copy_move_a2_false__int___int___int__(t184, t185, t186);
  __retval183 = r187;
  int* t188 = __retval183;
  return t188;
}

// function: _ZSt12__niter_baseIPiET_S1_
int* int__std____niter_base_int__(int* v189) {
bb190: ;
  int* __it191;
  int* __retval192;
  __it191 = v189;
  int* t193 = __it191;
  __retval192 = t193;
  int* t194 = __retval192;
  return t194;
}

// function: _ZNK9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEE4baseEv
int** __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______base___const(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v195) {
bb196: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* this197;
  int** __retval198;
  this197 = v195;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t199 = this197;
  __retval198 = &t199->_M_current;
  int** t200 = __retval198;
  return t200;
}

// function: _ZSt12__niter_baseIPiSt6vectorIiSaIiEEET_N9__gnu_cxx17__normal_iteratorIS4_T0_EE
int* int__std____niter_base_int___std__vector_int__std__allocator_int_____(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ v201) {
bb202: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __it203;
  int* __retval204;
  __it203 = v201;
  int** r205 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______base___const(&__it203);
  int* t206 = *r205;
  __retval204 = t206;
  int* t207 = __retval204;
  return t207;
}

// function: _ZSt13__copy_move_aILb0EPiN9__gnu_cxx17__normal_iteratorIS0_St6vectorIiSaIiEEEEET1_T0_S8_S7_
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____copy_move_a_false__int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(int* v208, int* v209, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ v210) {
bb211: ;
  int* __first212;
  int* __last213;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __result214;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __retval215;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ agg_tmp0216;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ agg_tmp1217;
  __first212 = v208;
  __last213 = v209;
  __result214 = v210;
  agg_tmp0216 = __result214; // copy
  int* t218 = __first212;
  int* r219 = int__std____niter_base_int__(t218);
  int* t220 = __last213;
  int* r221 = int__std____niter_base_int__(t220);
  agg_tmp1217 = __result214; // copy
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t222 = agg_tmp1217;
  int* r223 = int__std____niter_base_int___std__vector_int__std__allocator_int_____(t222);
  int* r224 = int__std____copy_move_a1_false__int___int__(r219, r221, r223);
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t225 = agg_tmp0216;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ r226 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____niter_wrap___gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______int__(t225, r224);
  __retval215 = r226;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t227 = __retval215;
  return t227;
}

// function: _ZSt12__miter_baseIPiET_S1_
int* int__std____miter_base_int__(int* v228) {
bb229: ;
  int* __it230;
  int* __retval231;
  __it230 = v228;
  int* t232 = __it230;
  __retval231 = t232;
  int* t233 = __retval231;
  return t233;
}

// function: _ZSt4copyIPiN9__gnu_cxx17__normal_iteratorIS0_St6vectorIiSaIiEEEEET0_T_S8_S7_
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std__copy_int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(int* v234, int* v235, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ v236) {
bb237: ;
  int* __first238;
  int* __last239;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __result240;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __retval241;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ agg_tmp0242;
  __first238 = v234;
  __last239 = v235;
  __result240 = v236;
  int* t243 = __first238;
  int* r244 = int__std____miter_base_int__(t243);
  int* t245 = __last239;
  int* r246 = int__std____miter_base_int__(t245);
  agg_tmp0242 = __result240; // copy
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t247 = agg_tmp0242;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ r248 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____copy_move_a_false__int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(r244, r246, t247);
  __retval241 = r248;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t249 = __retval241;
  return t249;
}

// function: _ZSt16__set_differenceIPiS0_N9__gnu_cxx17__normal_iteratorIS0_St6vectorIiSaIiEEEENS1_5__ops15_Iter_less_iterEET1_T_SA_T0_SB_S9_T2_
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____set_difference_int___int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________gnu_cxx____ops___Iter_less_iter_(int* v250, int* v251, int* v252, int* v253, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ v254, struct __gnu_cxx____ops___Iter_less_iter v255) {
bb256: ;
  int* __first1257;
  int* __last1258;
  int* __first2259;
  int* __last2260;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __result261;
  struct __gnu_cxx____ops___Iter_less_iter __comp262;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __retval263;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ agg_tmp0264;
  __first1257 = v250;
  __last1258 = v251;
  __first2259 = v252;
  __last2260 = v253;
  __result261 = v254;
  __comp262 = v255;
    while (1) {
      int* t265 = __first1257;
      int* t266 = __last1258;
      _Bool c267 = ((t265 != t266)) ? 1 : 0;
      _Bool ternary268;
      if (c267) {
        int* t269 = __first2259;
        int* t270 = __last2260;
        _Bool c271 = ((t269 != t270)) ? 1 : 0;
        ternary268 = (_Bool)c271;
      } else {
        _Bool c272 = 0;
        ternary268 = (_Bool)c272;
      }
      if (!ternary268) break;
        int* t273 = __first1257;
        int* t274 = __first2259;
        _Bool r275 = bool___gnu_cxx____ops___Iter_less_iter__operator___int___int___int___int___const(&__comp262, t273, t274);
        if (r275) {
          int* t276 = __first1257;
          int t277 = *t276;
          int* r278 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator____const(&__result261);
          *r278 = t277;
          int* t279 = __first1257;
          int c280 = 1;
          int* ptr281 = &(t279)[c280];
          __first1257 = ptr281;
          struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* r282 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator__(&__result261);
        } else {
            int* t283 = __first2259;
            int* t284 = __first1257;
            _Bool r285 = bool___gnu_cxx____ops___Iter_less_iter__operator___int___int___int___int___const(&__comp262, t283, t284);
            if (r285) {
              int* t286 = __first2259;
              int c287 = 1;
              int* ptr288 = &(t286)[c287];
              __first2259 = ptr288;
            } else {
              int* t289 = __first1257;
              int c290 = 1;
              int* ptr291 = &(t289)[c290];
              __first1257 = ptr291;
              int* t292 = __first2259;
              int c293 = 1;
              int* ptr294 = &(t292)[c293];
              __first2259 = ptr294;
            }
        }
    }
  int* t295 = __first1257;
  int* t296 = __last1258;
  agg_tmp0264 = __result261; // copy
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t297 = agg_tmp0264;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ r298 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std__copy_int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(t295, t296, t297);
  __retval263 = r298;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t299 = __retval263;
  return t299;
}

// function: _ZN9__gnu_cxx5__ops16__iter_less_iterEv
struct __gnu_cxx____ops___Iter_less_iter __gnu_cxx____ops____iter_less_iter() {
bb300: ;
  struct __gnu_cxx____ops___Iter_less_iter __retval301;
  struct __gnu_cxx____ops___Iter_less_iter t302 = __retval301;
  return t302;
}

// function: _ZSt14set_differenceIPiS0_N9__gnu_cxx17__normal_iteratorIS0_St6vectorIiSaIiEEEEET1_T_S8_T0_S9_S7_
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std__set_difference_int___int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(int* v303, int* v304, int* v305, int* v306, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ v307) {
bb308: ;
  int* __first1309;
  int* __last1310;
  int* __first2311;
  int* __last2312;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __result313;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __retval314;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ agg_tmp0315;
  struct __gnu_cxx____ops___Iter_less_iter agg_tmp1316;
  __first1309 = v303;
  __last1310 = v304;
  __first2311 = v305;
  __last2312 = v306;
  __result313 = v307;
  int* t317 = __first1309;
  int* t318 = __last1310;
  int* t319 = __first2311;
  int* t320 = __last2312;
  agg_tmp0315 = __result313; // copy
  struct __gnu_cxx____ops___Iter_less_iter r321 = __gnu_cxx____ops____iter_less_iter();
  agg_tmp1316 = r321;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t322 = agg_tmp0315;
  struct __gnu_cxx____ops___Iter_less_iter t323 = agg_tmp1316;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ r324 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std____set_difference_int___int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________gnu_cxx____ops___Iter_less_iter_(t317, t318, t319, t320, t322, t323);
  __retval314 = r324;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t325 = __retval314;
  return t325;
}

// function: _ZNSt6vectorIiSaIiEE5beginEv
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ std__vector_int__std__allocator_int_____begin(struct std__vector_int__std__allocator_int__* v326) {
bb327: ;
  struct std__vector_int__std__allocator_int__* this328;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ __retval329;
  this328 = v326;
  struct std__vector_int__std__allocator_int__* t330 = this328;
  struct std___Vector_base_int__std__allocator_int__* base331 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t330 + 0);
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base332 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base331->_M_impl) + 0);
  __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________normal_iterator(&__retval329, &base332->_M_start);
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t333 = __retval329;
  return t333;
}

// function: _ZN9__gnu_cxx17__normal_iteratorIPiSt6vectorIiSaIiEEEaSEOS5_
struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator_(struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v334, struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* v335) {
bb336: ;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* this337;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* unnamed338;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* __retval339;
  this337 = v334;
  unnamed338 = v335;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t340 = this337;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t341 = unnamed338;
  int* t342 = t341->_M_current;
  t340->_M_current = t342;
  __retval339 = t340;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* t343 = __retval339;
  return t343;
}

// function: _ZNKSt6vectorIiSaIiEE4sizeEv
unsigned long std__vector_int__std__allocator_int_____size___const(struct std__vector_int__std__allocator_int__* v344) {
bb345: ;
  struct std__vector_int__std__allocator_int__* this346;
  unsigned long __retval347;
  long __dif348;
  this346 = v344;
  struct std__vector_int__std__allocator_int__* t349 = this346;
  struct std___Vector_base_int__std__allocator_int__* base350 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t349 + 0);
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base351 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base350->_M_impl) + 0);
  int* t352 = base351->_M_finish;
  struct std___Vector_base_int__std__allocator_int__* base353 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t349 + 0);
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base354 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base353->_M_impl) + 0);
  int* t355 = base354->_M_start;
  long diff356 = t352 - t355;
  __dif348 = diff356;
    long t357 = __dif348;
    long c358 = 0;
    _Bool c359 = ((t357 < c358)) ? 1 : 0;
    if (c359) {
      abort();
    }
  long t360 = __dif348;
  unsigned long cast361 = (unsigned long)t360;
  __retval347 = cast361;
  unsigned long t362 = __retval347;
  return t362;
}

// function: _ZNSt6vectorIiSaIiEEixEm
int* std__vector_int__std__allocator_int_____operator__(struct std__vector_int__std__allocator_int__* v363, unsigned long v364) {
bb365: ;
  struct std__vector_int__std__allocator_int__* this366;
  unsigned long __n367;
  int* __retval368;
  this366 = v363;
  __n367 = v364;
  struct std__vector_int__std__allocator_int__* t369 = this366;
    do {
          unsigned long t370 = __n367;
          unsigned long r371 = std__vector_int__std__allocator_int_____size___const(t369);
          _Bool c372 = ((t370 < r371)) ? 1 : 0;
          _Bool u373 = !c372;
          if (u373) {
            char* cast374 = (char*)&(_str_5);
            int c375 = 1263;
            char* cast376 = (char*)&(__PRETTY_FUNCTION____ZNSt6vectorIiSaIiEEixEm);
            char* cast377 = (char*)&(_str_6);
            std____glibcxx_assert_fail(cast374, c375, cast376, cast377);
          }
      _Bool c378 = 0;
      if (!c378) break;
    } while (1);
  struct std___Vector_base_int__std__allocator_int__* base379 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t369 + 0);
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base380 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base379->_M_impl) + 0);
  int* t381 = base380->_M_start;
  unsigned long t382 = __n367;
  int* ptr383 = &(t381)[t382];
  __retval368 = ptr383;
  int* t384 = __retval368;
  return t384;
}

// function: _ZNSt6vectorIiSaIiEED2Ev
void std__vector_int__std__allocator_int______vector(struct std__vector_int__std__allocator_int__* v385) {
bb386: ;
  struct std__vector_int__std__allocator_int__* this387;
  this387 = v385;
  struct std__vector_int__std__allocator_int__* t388 = this387;
    struct std___Vector_base_int__std__allocator_int__* base389 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t388 + 0);
    struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base390 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base389->_M_impl) + 0);
    int* t391 = base390->_M_start;
    struct std___Vector_base_int__std__allocator_int__* base392 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t388 + 0);
    struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base393 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base392->_M_impl) + 0);
    int* t394 = base393->_M_finish;
    struct std___Vector_base_int__std__allocator_int__* base395 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t388 + 0);
    struct std__allocator_int_* r396 = std___Vector_base_int__std__allocator_int______M_get_Tp_allocator(base395);
    void_std___Destroy_int___int_(t391, t394, r396);
  {
    struct std___Vector_base_int__std__allocator_int__* base397 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t388 + 0);
    std___Vector_base_int__std__allocator_int_______Vector_base(base397);
  }
  return;
}

// function: main
int main() {
bb398: ;
  int __retval399;
  int first400[5];
  int second401[5];
  struct std__vector_int__std__allocator_int__ v402;
  struct std__allocator_int_ ref_tmp0403;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ it404;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ ref_tmp1405;
  struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ agg_tmp0406;
  int c407 = 0;
  __retval399 = c407;
  // array copy
  memcpy(first400, __const_main_first, (unsigned long)5 * sizeof(__const_main_first[0]));
  // array copy
  memcpy(second401, __const_main_second, (unsigned long)5 * sizeof(__const_main_second[0]));
  unsigned long c408 = 10;
  std__allocator_int___allocator_2(&ref_tmp0403);
    std__vector_int__std__allocator_int_____vector(&v402, c408, &ref_tmp0403);
  {
    std__allocator_int____allocator(&ref_tmp0403);
  }
    __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_________normal_iterator_2(&it404);
    int* cast409 = (int*)&(first400);
    int* cast410 = (int*)&(first400);
    int c411 = 5;
    int* ptr412 = &(cast410)[c411];
    int* cast413 = (int*)&(second401);
    int* cast414 = (int*)&(second401);
    int c415 = 5;
    int* ptr416 = &(cast414)[c415];
    struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ r417 = std__vector_int__std__allocator_int_____begin(&v402);
    agg_tmp0406 = r417;
    struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ t418 = agg_tmp0406;
    struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___ r419 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int______std__set_difference_int___int_____gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______(cast409, ptr412, cast413, ptr416, t418);
    ref_tmp1405 = r419;
    struct __gnu_cxx____normal_iterator_int____std__vector_int__std__allocator_int___* r420 = __gnu_cxx____normal_iterator_int___std__vector_int__std__allocator_int_______operator_(&it404, &ref_tmp1405);
    unsigned long c421 = 0;
    int* r422 = std__vector_int__std__allocator_int_____operator__(&v402, c421);
    int t423 = *r422;
    int c424 = 5;
    _Bool c425 = ((t423 == c424)) ? 1 : 0;
    if (c425) {
    } else {
      char* cast426 = (char*)&(_str);
      char* c427 = (char*)_str_1;
      unsigned int c428 = 23;
      char* cast429 = (char*)&(__PRETTY_FUNCTION___main);
      reach_error();
      __assert_fail(cast426, c427, c428, cast429);
    }
    unsigned long c430 = 1;
    int* r431 = std__vector_int__std__allocator_int_____operator__(&v402, c430);
    int t432 = *r431;
    int c433 = 15;
    _Bool c434 = ((t432 == c433)) ? 1 : 0;
    if (c434) {
    } else {
      char* cast435 = (char*)&(_str_2);
      char* c436 = (char*)_str_1;
      unsigned int c437 = 24;
      char* cast438 = (char*)&(__PRETTY_FUNCTION___main);
      reach_error();
      __assert_fail(cast435, c436, c437, cast438);
    }
    unsigned long c439 = 2;
    int* r440 = std__vector_int__std__allocator_int_____operator__(&v402, c439);
    int t441 = *r440;
    int c442 = 25;
    _Bool c443 = ((t441 == c442)) ? 1 : 0;
    if (c443) {
    } else {
      char* cast444 = (char*)&(_str_3);
      char* c445 = (char*)_str_1;
      unsigned int c446 = 25;
      char* cast447 = (char*)&(__PRETTY_FUNCTION___main);
      reach_error();
      __assert_fail(cast444, c445, c446, cast447);
    }
    int c448 = 0;
    __retval399 = c448;
    int t449 = __retval399;
    int ret_val450 = t449;
    {
      std__vector_int__std__allocator_int______vector(&v402);
    }
    return ret_val450;
  int t451 = __retval399;
  return t451;
}

// function: _ZNSt15__new_allocatorIiEC2Ev
void std____new_allocator_int_____new_allocator_2(struct std____new_allocator_int_* v452) {
bb453: ;
  struct std____new_allocator_int_* this454;
  this454 = v452;
  struct std____new_allocator_int_* t455 = this454;
  return;
}

// function: _ZSt3minImERKT_S2_S2_
unsigned long* unsigned_long_const__std__min_unsigned_long_(unsigned long* v456, unsigned long* v457) {
bb458: ;
  unsigned long* __a459;
  unsigned long* __b460;
  unsigned long* __retval461;
  __a459 = v456;
  __b460 = v457;
    unsigned long* t462 = __b460;
    unsigned long t463 = *t462;
    unsigned long* t464 = __a459;
    unsigned long t465 = *t464;
    _Bool c466 = ((t463 < t465)) ? 1 : 0;
    if (c466) {
      unsigned long* t467 = __b460;
      __retval461 = t467;
      unsigned long* t468 = __retval461;
      return t468;
    }
  unsigned long* t469 = __a459;
  __retval461 = t469;
  unsigned long* t470 = __retval461;
  return t470;
}

// function: _ZNSt6vectorIiSaIiEE11_S_max_sizeERKS0_
unsigned long std__vector_int__std__allocator_int______S_max_size(struct std__allocator_int_* v471) {
bb472: ;
  struct std__allocator_int_* __a473;
  unsigned long __retval474;
  unsigned long __diffmax475;
  unsigned long __allocmax476;
  __a473 = v471;
  unsigned long c477 = 2305843009213693951;
  __diffmax475 = c477;
  unsigned long c478 = 4611686018427387903;
  __allocmax476 = c478;
  unsigned long* r479 = unsigned_long_const__std__min_unsigned_long_(&__diffmax475, &__allocmax476);
  unsigned long t480 = *r479;
  __retval474 = t480;
  unsigned long t481 = __retval474;
  return t481;
}

// function: _ZNSaIiEC2ERKS_
void std__allocator_int___allocator(struct std__allocator_int_* v482, struct std__allocator_int_* v483) {
bb484: ;
  struct std__allocator_int_* this485;
  struct std__allocator_int_* __a486;
  this485 = v482;
  __a486 = v483;
  struct std__allocator_int_* t487 = this485;
  struct std____new_allocator_int_* base488 = (struct std____new_allocator_int_*)((char *)t487 + 0);
  struct std__allocator_int_* t489 = __a486;
  struct std____new_allocator_int_* base490 = (struct std____new_allocator_int_*)((char *)t489 + 0);
  std____new_allocator_int_____new_allocator(base488, base490);
  return;
}

// function: _ZNSt6vectorIiSaIiEE17_S_check_init_lenEmRKS0_
unsigned long std__vector_int__std__allocator_int______S_check_init_len(unsigned long v491, struct std__allocator_int_* v492) {
bb493: ;
  unsigned long __n494;
  struct std__allocator_int_* __a495;
  unsigned long __retval496;
  __n494 = v491;
  __a495 = v492;
    struct std__allocator_int_ ref_tmp0497;
    _Bool tmp_exprcleanup498;
    unsigned long t499 = __n494;
    struct std__allocator_int_* t500 = __a495;
    std__allocator_int___allocator(&ref_tmp0497, t500);
      unsigned long r501 = std__vector_int__std__allocator_int______S_max_size(&ref_tmp0497);
      _Bool c502 = ((t499 > r501)) ? 1 : 0;
      tmp_exprcleanup498 = c502;
    {
      std__allocator_int____allocator(&ref_tmp0497);
    }
    _Bool t503 = tmp_exprcleanup498;
    if (t503) {
      char* cast504 = (char*)&(_str_4);
      std____throw_length_error(cast504);
    }
  unsigned long t505 = __n494;
  __retval496 = t505;
  unsigned long t506 = __retval496;
  return t506;
}

// function: _ZNSt12_Vector_baseIiSaIiEE12_Vector_implC2ERKS0_
void std___Vector_base_int__std__allocator_int______Vector_impl___Vector_impl(struct std___Vector_base_int__std__allocator_int_____Vector_impl* v507, struct std__allocator_int_* v508) {
bb509: ;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl* this510;
  struct std__allocator_int_* __a511;
  this510 = v507;
  __a511 = v508;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl* t512 = this510;
  struct std__allocator_int_* base513 = (struct std__allocator_int_*)((char *)t512 + 0);
  struct std__allocator_int_* t514 = __a511;
  std__allocator_int___allocator(base513, t514);
    struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base515 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)t512 + 0);
    std___Vector_base_int__std__allocator_int______Vector_impl_data___Vector_impl_data(base515);
  return;
}

// function: _ZSt23__is_constant_evaluatedv
_Bool std____is_constant_evaluated() {
bb516: ;
  _Bool __retval517;
    _Bool c518 = 0;
    __retval517 = c518;
    _Bool t519 = __retval517;
    return t519;
  abort();
}

// function: _ZNKSt15__new_allocatorIiE11_M_max_sizeEv
unsigned long std____new_allocator_int____M_max_size___const(struct std____new_allocator_int_* v520) {
bb521: ;
  struct std____new_allocator_int_* this522;
  unsigned long __retval523;
  this522 = v520;
  struct std____new_allocator_int_* t524 = this522;
  unsigned long c525 = 9223372036854775807;
  unsigned long c526 = 4;
  unsigned long b527 = c525 / c526;
  __retval523 = b527;
  unsigned long t528 = __retval523;
  return t528;
}

// function: _ZNSt15__new_allocatorIiE8allocateEmPKv
int* std____new_allocator_int___allocate(struct std____new_allocator_int_* v529, unsigned long v530, void* v531) {
bb532: ;
  struct std____new_allocator_int_* this533;
  unsigned long __n534;
  void* unnamed535;
  int* __retval536;
  this533 = v529;
  __n534 = v530;
  unnamed535 = v531;
  struct std____new_allocator_int_* t537 = this533;
    unsigned long t538 = __n534;
    unsigned long r539 = std____new_allocator_int____M_max_size___const(t537);
    _Bool c540 = ((t538 > r539)) ? 1 : 0;
    if (c540) {
        unsigned long t541 = __n534;
        unsigned long c542 = -1;
        unsigned long c543 = 4;
        unsigned long b544 = c542 / c543;
        _Bool c545 = ((t541 > b544)) ? 1 : 0;
        if (c545) {
          std____throw_bad_array_new_length();
        }
      std____throw_bad_alloc();
    }
    unsigned long c546 = 4;
    unsigned long c547 = 16;
    _Bool c548 = ((c546 > c547)) ? 1 : 0;
    if (c548) {
      unsigned long __al549;
      unsigned long c550 = 4;
      __al549 = c550;
      unsigned long t551 = __n534;
      unsigned long c552 = 4;
      unsigned long b553 = t551 * c552;
      unsigned long t554 = __al549;
      void* r555 = operator_new_2(b553, t554);
      int* cast556 = (int*)r555;
      __retval536 = cast556;
      int* t557 = __retval536;
      return t557;
    }
  unsigned long t558 = __n534;
  unsigned long c559 = 4;
  unsigned long b560 = t558 * c559;
  void* r561 = operator_new(b560);
  int* cast562 = (int*)r561;
  __retval536 = cast562;
  int* t563 = __retval536;
  return t563;
}

// function: _ZNSaIiE8allocateEm
int* std__allocator_int___allocate(struct std__allocator_int_* v564, unsigned long v565) {
bb566: ;
  struct std__allocator_int_* this567;
  unsigned long __n568;
  int* __retval569;
  this567 = v564;
  __n568 = v565;
  struct std__allocator_int_* t570 = this567;
    _Bool r571 = std____is_constant_evaluated();
    if (r571) {
        unsigned long t572 = __n568;
        unsigned long c573 = 4;
        unsigned long ovr574;
        _Bool ovf575 = __builtin_mul_overflow(t572, c573, &ovr574);
        __n568 = ovr574;
        if (ovf575) {
          std____throw_bad_array_new_length();
        }
      unsigned long t576 = __n568;
      void* r577 = operator_new(t576);
      int* cast578 = (int*)r577;
      __retval569 = cast578;
      int* t579 = __retval569;
      return t579;
    }
  struct std____new_allocator_int_* base580 = (struct std____new_allocator_int_*)((char *)t570 + 0);
  unsigned long t581 = __n568;
  void* c582 = ((void*)0);
  int* r583 = std____new_allocator_int___allocate(base580, t581, c582);
  __retval569 = r583;
  int* t584 = __retval569;
  return t584;
}

// function: _ZNSt16allocator_traitsISaIiEE8allocateERS0_m
int* std__allocator_traits_std__allocator_int_____allocate(struct std__allocator_int_* v585, unsigned long v586) {
bb587: ;
  struct std__allocator_int_* __a588;
  unsigned long __n589;
  int* __retval590;
  __a588 = v585;
  __n589 = v586;
  struct std__allocator_int_* t591 = __a588;
  unsigned long t592 = __n589;
  int* r593 = std__allocator_int___allocate(t591, t592);
  __retval590 = r593;
  int* t594 = __retval590;
  return t594;
}

// function: _ZNSt12_Vector_baseIiSaIiEE11_M_allocateEm
int* std___Vector_base_int__std__allocator_int______M_allocate(struct std___Vector_base_int__std__allocator_int__* v595, unsigned long v596) {
bb597: ;
  struct std___Vector_base_int__std__allocator_int__* this598;
  unsigned long __n599;
  int* __retval600;
  this598 = v595;
  __n599 = v596;
  struct std___Vector_base_int__std__allocator_int__* t601 = this598;
  unsigned long t602 = __n599;
  unsigned long c603 = 0;
  _Bool c604 = ((t602 != c603)) ? 1 : 0;
  int* ternary605;
  if (c604) {
    struct std__allocator_int_* base606 = (struct std__allocator_int_*)((char *)&(t601->_M_impl) + 0);
    unsigned long t607 = __n599;
    int* r608 = std__allocator_traits_std__allocator_int_____allocate(base606, t607);
    ternary605 = (int*)r608;
  } else {
    int* c609 = ((void*)0);
    ternary605 = (int*)c609;
  }
  __retval600 = ternary605;
  int* t610 = __retval600;
  return t610;
}

// function: _ZNSt12_Vector_baseIiSaIiEE17_M_create_storageEm
void std___Vector_base_int__std__allocator_int______M_create_storage(struct std___Vector_base_int__std__allocator_int__* v611, unsigned long v612) {
bb613: ;
  struct std___Vector_base_int__std__allocator_int__* this614;
  unsigned long __n615;
  this614 = v611;
  __n615 = v612;
  struct std___Vector_base_int__std__allocator_int__* t616 = this614;
  unsigned long t617 = __n615;
  int* r618 = std___Vector_base_int__std__allocator_int______M_allocate(t616, t617);
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base619 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t616->_M_impl) + 0);
  base619->_M_start = r618;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base620 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t616->_M_impl) + 0);
  int* t621 = base620->_M_start;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base622 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t616->_M_impl) + 0);
  base622->_M_finish = t621;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base623 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t616->_M_impl) + 0);
  int* t624 = base623->_M_start;
  unsigned long t625 = __n615;
  int* ptr626 = &(t624)[t625];
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base627 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t616->_M_impl) + 0);
  base627->_M_end_of_storage = ptr626;
  return;
}

// function: _ZNSt12_Vector_baseIiSaIiEE12_Vector_implD2Ev
void std___Vector_base_int__std__allocator_int______Vector_impl____Vector_impl(struct std___Vector_base_int__std__allocator_int_____Vector_impl* v628) {
bb629: ;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl* this630;
  this630 = v628;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl* t631 = this630;
  {
    struct std__allocator_int_* base632 = (struct std__allocator_int_*)((char *)t631 + 0);
    std__allocator_int____allocator(base632);
  }
  return;
}

// function: _ZNSt12_Vector_baseIiSaIiEEC2EmRKS0_
void std___Vector_base_int__std__allocator_int______Vector_base(struct std___Vector_base_int__std__allocator_int__* v633, unsigned long v634, struct std__allocator_int_* v635) {
bb636: ;
  struct std___Vector_base_int__std__allocator_int__* this637;
  unsigned long __n638;
  struct std__allocator_int_* __a639;
  this637 = v633;
  __n638 = v634;
  __a639 = v635;
  struct std___Vector_base_int__std__allocator_int__* t640 = this637;
  struct std__allocator_int_* t641 = __a639;
  std___Vector_base_int__std__allocator_int______Vector_impl___Vector_impl(&t640->_M_impl, t641);
    unsigned long t642 = __n638;
    std___Vector_base_int__std__allocator_int______M_create_storage(t640, t642);
  return;
}

// function: _ZSt21is_constant_evaluatedv
_Bool std__is_constant_evaluated() {
bb643: ;
  _Bool __retval644;
    _Bool c645 = 0;
    __retval644 = c645;
    _Bool t646 = __retval644;
    return t646;
  abort();
}

// function: _ZNSt19_UninitDestroyGuardIPivEC2ERS0_
void std___UninitDestroyGuard_int___void____UninitDestroyGuard(struct std___UninitDestroyGuard_int____void_* v647, int** v648) {
bb649: ;
  struct std___UninitDestroyGuard_int____void_* this650;
  int** __first651;
  this650 = v647;
  __first651 = v648;
  struct std___UninitDestroyGuard_int____void_* t652 = this650;
  int** t653 = __first651;
  int* t654 = *t653;
  t652->_M_first = t654;
  int** t655 = __first651;
  t652->_M_cur = t655;
  return;
}

// function: _ZSt12construct_atIiJEQaant20is_unbounded_array_vIT_ErqXgsnwcvPvLi0E_S0_pispclsr3stdE7declvalIT0_EEEEEPS0_S3_DpOS2_
int* _ZSt12construct_atIiJEQaant20is_unbounded_array_vIT_ErqXgsnwcvPvLi0E_S0_pispclsr3stdE7declvalIT0_EEEEEPS0_S3_DpOS2_(int* v656) {
bb657: ;
  int* __location658;
  int* __retval659;
  void* __loc660;
  __location658 = v656;
  int* t661 = __location658;
  void* cast662 = (void*)t661;
  __loc660 = cast662;
    void* t663 = __loc660;
    int* cast664 = (int*)t663;
    int c665 = 0;
    *cast664 = c665;
    __retval659 = cast664;
    int* t666 = __retval659;
    return t666;
  abort();
}

// function: _ZSt10_ConstructIiJEEvPT_DpOT0_
void void_std___Construct_int_(int* v667) {
bb668: ;
  int* __p669;
  __p669 = v667;
    _Bool r670 = std____is_constant_evaluated();
    if (r670) {
      int* t671 = __p669;
      int* r672 = _ZSt12construct_atIiJEQaant20is_unbounded_array_vIT_ErqXgsnwcvPvLi0E_S0_pispclsr3stdE7declvalIT0_EEEEEPS0_S3_DpOS2_(t671);
      return;
    }
  int* t673 = __p669;
  void* cast674 = (void*)t673;
  int* cast675 = (int*)cast674;
  int c676 = 0;
  *cast675 = c676;
  return;
}

// function: _ZNSt19_UninitDestroyGuardIPivE7releaseEv
void std___UninitDestroyGuard_int___void___release(struct std___UninitDestroyGuard_int____void_* v677) {
bb678: ;
  struct std___UninitDestroyGuard_int____void_* this679;
  this679 = v677;
  struct std___UninitDestroyGuard_int____void_* t680 = this679;
  int** c681 = ((void*)0);
  t680->_M_cur = c681;
  return;
}

// function: _ZNSt19_UninitDestroyGuardIPivED2Ev
void std___UninitDestroyGuard_int___void_____UninitDestroyGuard(struct std___UninitDestroyGuard_int____void_* v682) {
bb683: ;
  struct std___UninitDestroyGuard_int____void_* this684;
  this684 = v682;
  struct std___UninitDestroyGuard_int____void_* t685 = this684;
    int** t686 = t685->_M_cur;
    int** c687 = ((void*)0);
    _Bool c688 = ((t686 != c687)) ? 1 : 0;
    if (c688) {
      int* t689 = t685->_M_first;
      int** t690 = t685->_M_cur;
      int* t691 = *t690;
      void_std___Destroy_int__(t689, t691);
    }
  return;
}

// function: _ZNSt27__uninitialized_default_n_1ILb0EE18__uninit_default_nIPimEET_S3_T0_
int* int__std____uninitialized_default_n_1_false_____uninit_default_n_int___unsigned_long_(int* v692, unsigned long v693) {
bb694: ;
  int* __first695;
  unsigned long __n696;
  int* __retval697;
  struct std___UninitDestroyGuard_int____void_ __guard698;
  __first695 = v692;
  __n696 = v693;
  std___UninitDestroyGuard_int___void____UninitDestroyGuard(&__guard698, &__first695);
      while (1) {
        unsigned long t700 = __n696;
        unsigned long c701 = 0;
        _Bool c702 = ((t700 > c701)) ? 1 : 0;
        if (!c702) break;
        int* t703 = __first695;
        void_std___Construct_int_(t703);
      for_step699: ;
        unsigned long t704 = __n696;
        unsigned long u705 = t704 - 1;
        __n696 = u705;
        int* t706 = __first695;
        int c707 = 1;
        int* ptr708 = &(t706)[c707];
        __first695 = ptr708;
      }
    std___UninitDestroyGuard_int___void___release(&__guard698);
    int* t709 = __first695;
    __retval697 = t709;
    int* t710 = __retval697;
    int* ret_val711 = t710;
    {
      std___UninitDestroyGuard_int___void_____UninitDestroyGuard(&__guard698);
    }
    return ret_val711;
  abort();
}

// function: _ZSt9__fill_a1IPiiEvT_S1_RKT0_
void void_std____fill_a1_int___int_(int* v712, int* v713, int* v714) {
bb715: ;
  int* __first716;
  int* __last717;
  int* __value718;
  _Bool __load_outside_loop719;
  int __val720;
  __first716 = v712;
  __last717 = v713;
  __value718 = v714;
  _Bool c721 = 1;
  __load_outside_loop719 = c721;
  int* t722 = __value718;
  int t723 = *t722;
  __val720 = t723;
    while (1) {
      int* t725 = __first716;
      int* t726 = __last717;
      _Bool c727 = ((t725 != t726)) ? 1 : 0;
      if (!c727) break;
      int t728 = __val720;
      int* t729 = __first716;
      *t729 = t728;
    for_step724: ;
      int* t730 = __first716;
      int c731 = 1;
      int* ptr732 = &(t730)[c731];
      __first716 = ptr732;
    }
  return;
}

// function: _ZSt8__fill_aIPiiEvT_S1_RKT0_
void void_std____fill_a_int___int_(int* v733, int* v734, int* v735) {
bb736: ;
  int* __first737;
  int* __last738;
  int* __value739;
  __first737 = v733;
  __last738 = v734;
  __value739 = v735;
  int* t740 = __first737;
  int* t741 = __last738;
  int* t742 = __value739;
  void_std____fill_a1_int___int_(t740, t741, t742);
  return;
}

// function: _ZSt10__fill_n_aIPimiET_S1_T0_RKT1_St26random_access_iterator_tag
int* int__std____fill_n_a_int___unsigned_long__int_(int* v743, unsigned long v744, int* v745, struct std__random_access_iterator_tag v746) {
bb747: ;
  int* __first748;
  unsigned long __n749;
  int* __value750;
  struct std__random_access_iterator_tag unnamed751;
  int* __retval752;
  __first748 = v743;
  __n749 = v744;
  __value750 = v745;
  unnamed751 = v746;
    unsigned long t753 = __n749;
    unsigned long c754 = 0;
    _Bool c755 = ((t753 <= c754)) ? 1 : 0;
    if (c755) {
      int* t756 = __first748;
      __retval752 = t756;
      int* t757 = __retval752;
      return t757;
    }
  int* t758 = __first748;
  int* t759 = __first748;
  unsigned long t760 = __n749;
  int* ptr761 = &(t759)[t760];
  int* t762 = __value750;
  void_std____fill_a_int___int_(t758, ptr761, t762);
  int* t763 = __first748;
  unsigned long t764 = __n749;
  int* ptr765 = &(t763)[t764];
  __retval752 = ptr765;
  int* t766 = __retval752;
  return t766;
}

// function: _ZSt17__size_to_integerm
unsigned long std____size_to_integer(unsigned long v767) {
bb768: ;
  unsigned long __n769;
  unsigned long __retval770;
  __n769 = v767;
  unsigned long t771 = __n769;
  __retval770 = t771;
  unsigned long t772 = __retval770;
  return t772;
}

// function: _ZSt19__iterator_categoryIPiENSt15iterator_traitsIT_E17iterator_categoryERKS2_
struct std__random_access_iterator_tag std__iterator_traits_int____iterator_category_std____iterator_category_int__(int** v773) {
bb774: ;
  int** unnamed775;
  struct std__random_access_iterator_tag __retval776;
  unnamed775 = v773;
  struct std__random_access_iterator_tag t777 = __retval776;
  return t777;
}

// function: _ZSt6fill_nIPimiET_S1_T0_RKT1_
int* int__std__fill_n_int___unsigned_long__int_(int* v778, unsigned long v779, int* v780) {
bb781: ;
  int* __first782;
  unsigned long __n783;
  int* __value784;
  int* __retval785;
  struct std__random_access_iterator_tag agg_tmp0786;
  __first782 = v778;
  __n783 = v779;
  __value784 = v780;
  int* t787 = __first782;
  unsigned long t788 = __n783;
  unsigned long r789 = std____size_to_integer(t788);
  int* t790 = __value784;
  struct std__random_access_iterator_tag r791 = std__iterator_traits_int____iterator_category_std____iterator_category_int__(&__first782);
  agg_tmp0786 = r791;
  struct std__random_access_iterator_tag t792 = agg_tmp0786;
  int* r793 = int__std____fill_n_a_int___unsigned_long__int_(t787, r789, t790, t792);
  __retval785 = r793;
  int* t794 = __retval785;
  return t794;
}

// function: _ZNSt27__uninitialized_default_n_1ILb1EE18__uninit_default_nIPimEET_S3_T0_
int* int__std____uninitialized_default_n_1_true_____uninit_default_n_int___unsigned_long_(int* v795, unsigned long v796) {
bb797: ;
  int* __first798;
  unsigned long __n799;
  int* __retval800;
  __first798 = v795;
  __n799 = v796;
    unsigned long t801 = __n799;
    unsigned long c802 = 0;
    _Bool c803 = ((t801 > c802)) ? 1 : 0;
    if (c803) {
      int* __val804;
      int* t805 = __first798;
      __val804 = t805;
      int* t806 = __val804;
      void_std___Construct_int_(t806);
      int* t807 = __first798;
      int c808 = 1;
      int* ptr809 = &(t807)[c808];
      __first798 = ptr809;
      int* t810 = __first798;
      unsigned long t811 = __n799;
      unsigned long c812 = 1;
      unsigned long b813 = t811 - c812;
      int* t814 = __val804;
      int* r815 = int__std__fill_n_int___unsigned_long__int_(t810, b813, t814);
      __first798 = r815;
    }
  int* t816 = __first798;
  __retval800 = t816;
  int* t817 = __retval800;
  return t817;
}

// function: _ZSt25__uninitialized_default_nIPimET_S1_T0_
int* int__std____uninitialized_default_n_int___unsigned_long_(int* v818, unsigned long v819) {
bb820: ;
  int* __first821;
  unsigned long __n822;
  int* __retval823;
  _Bool __can_fill824;
  __first821 = v818;
  __n822 = v819;
    _Bool r825 = std__is_constant_evaluated();
    if (r825) {
      int* t826 = __first821;
      unsigned long t827 = __n822;
      int* r828 = int__std____uninitialized_default_n_1_false_____uninit_default_n_int___unsigned_long_(t826, t827);
      __retval823 = r828;
      int* t829 = __retval823;
      return t829;
    }
  _Bool c830 = 1;
  __can_fill824 = c830;
  int* t831 = __first821;
  unsigned long t832 = __n822;
  int* r833 = int__std____uninitialized_default_n_1_true_____uninit_default_n_int___unsigned_long_(t831, t832);
  __retval823 = r833;
  int* t834 = __retval823;
  return t834;
}

// function: _ZSt27__uninitialized_default_n_aIPimiET_S1_T0_RSaIT1_E
int* int__std____uninitialized_default_n_a_int___unsigned_long__int_(int* v835, unsigned long v836, struct std__allocator_int_* v837) {
bb838: ;
  int* __first839;
  unsigned long __n840;
  struct std__allocator_int_* unnamed841;
  int* __retval842;
  __first839 = v835;
  __n840 = v836;
  unnamed841 = v837;
  int* t843 = __first839;
  unsigned long t844 = __n840;
  int* r845 = int__std____uninitialized_default_n_int___unsigned_long_(t843, t844);
  __retval842 = r845;
  int* t846 = __retval842;
  return t846;
}

// function: _ZNSt12_Vector_baseIiSaIiEE19_M_get_Tp_allocatorEv
struct std__allocator_int_* std___Vector_base_int__std__allocator_int______M_get_Tp_allocator(struct std___Vector_base_int__std__allocator_int__* v847) {
bb848: ;
  struct std___Vector_base_int__std__allocator_int__* this849;
  struct std__allocator_int_* __retval850;
  this849 = v847;
  struct std___Vector_base_int__std__allocator_int__* t851 = this849;
  struct std__allocator_int_* base852 = (struct std__allocator_int_*)((char *)&(t851->_M_impl) + 0);
  __retval850 = base852;
  struct std__allocator_int_* t853 = __retval850;
  return t853;
}

// function: _ZNSt6vectorIiSaIiEE21_M_default_initializeEm
void std__vector_int__std__allocator_int______M_default_initialize(struct std__vector_int__std__allocator_int__* v854, unsigned long v855) {
bb856: ;
  struct std__vector_int__std__allocator_int__* this857;
  unsigned long __n858;
  this857 = v854;
  __n858 = v855;
  struct std__vector_int__std__allocator_int__* t859 = this857;
  struct std___Vector_base_int__std__allocator_int__* base860 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t859 + 0);
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base861 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base860->_M_impl) + 0);
  int* t862 = base861->_M_start;
  unsigned long t863 = __n858;
  struct std___Vector_base_int__std__allocator_int__* base864 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t859 + 0);
  struct std__allocator_int_* r865 = std___Vector_base_int__std__allocator_int______M_get_Tp_allocator(base864);
  int* r866 = int__std____uninitialized_default_n_a_int___unsigned_long__int_(t862, t863, r865);
  struct std___Vector_base_int__std__allocator_int__* base867 = (struct std___Vector_base_int__std__allocator_int__*)((char *)t859 + 0);
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base868 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(base867->_M_impl) + 0);
  base868->_M_finish = r866;
  return;
}

// function: _ZNSt15__new_allocatorIiE10deallocateEPim
void std____new_allocator_int___deallocate(struct std____new_allocator_int_* v869, int* v870, unsigned long v871) {
bb872: ;
  struct std____new_allocator_int_* this873;
  int* __p874;
  unsigned long __n875;
  this873 = v869;
  __p874 = v870;
  __n875 = v871;
  struct std____new_allocator_int_* t876 = this873;
    unsigned long c877 = 4;
    unsigned long c878 = 16;
    _Bool c879 = ((c877 > c878)) ? 1 : 0;
    if (c879) {
      int* t880 = __p874;
      void* cast881 = (void*)t880;
      unsigned long t882 = __n875;
      unsigned long c883 = 4;
      unsigned long b884 = t882 * c883;
      unsigned long c885 = 4;
      operator_delete_3(cast881, b884, c885);
      return;
    }
  int* t886 = __p874;
  void* cast887 = (void*)t886;
  unsigned long t888 = __n875;
  unsigned long c889 = 4;
  unsigned long b890 = t888 * c889;
  operator_delete_2(cast887, b890);
  return;
}

// function: _ZNSaIiE10deallocateEPim
void std__allocator_int___deallocate(struct std__allocator_int_* v891, int* v892, unsigned long v893) {
bb894: ;
  struct std__allocator_int_* this895;
  int* __p896;
  unsigned long __n897;
  this895 = v891;
  __p896 = v892;
  __n897 = v893;
  struct std__allocator_int_* t898 = this895;
    _Bool r899 = std____is_constant_evaluated();
    if (r899) {
      int* t900 = __p896;
      void* cast901 = (void*)t900;
      operator_delete(cast901);
      return;
    }
  struct std____new_allocator_int_* base902 = (struct std____new_allocator_int_*)((char *)t898 + 0);
  int* t903 = __p896;
  unsigned long t904 = __n897;
  std____new_allocator_int___deallocate(base902, t903, t904);
  return;
}

// function: _ZNSt16allocator_traitsISaIiEE10deallocateERS0_Pim
void std__allocator_traits_std__allocator_int_____deallocate(struct std__allocator_int_* v905, int* v906, unsigned long v907) {
bb908: ;
  struct std__allocator_int_* __a909;
  int* __p910;
  unsigned long __n911;
  __a909 = v905;
  __p910 = v906;
  __n911 = v907;
  struct std__allocator_int_* t912 = __a909;
  int* t913 = __p910;
  unsigned long t914 = __n911;
  std__allocator_int___deallocate(t912, t913, t914);
  return;
}

// function: _ZNSt12_Vector_baseIiSaIiEE13_M_deallocateEPim
void std___Vector_base_int__std__allocator_int______M_deallocate(struct std___Vector_base_int__std__allocator_int__* v915, int* v916, unsigned long v917) {
bb918: ;
  struct std___Vector_base_int__std__allocator_int__* this919;
  int* __p920;
  unsigned long __n921;
  this919 = v915;
  __p920 = v916;
  __n921 = v917;
  struct std___Vector_base_int__std__allocator_int__* t922 = this919;
    int* t923 = __p920;
    _Bool cast924 = (_Bool)t923;
    if (cast924) {
      struct std__allocator_int_* base925 = (struct std__allocator_int_*)((char *)&(t922->_M_impl) + 0);
      int* t926 = __p920;
      unsigned long t927 = __n921;
      std__allocator_traits_std__allocator_int_____deallocate(base925, t926, t927);
    }
  return;
}

// function: _ZNSt12_Vector_baseIiSaIiEED2Ev
void std___Vector_base_int__std__allocator_int_______Vector_base(struct std___Vector_base_int__std__allocator_int__* v928) {
bb929: ;
  struct std___Vector_base_int__std__allocator_int__* this930;
  this930 = v928;
  struct std___Vector_base_int__std__allocator_int__* t931 = this930;
    struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base932 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t931->_M_impl) + 0);
    int* t933 = base932->_M_start;
    struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base934 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t931->_M_impl) + 0);
    int* t935 = base934->_M_end_of_storage;
    struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* base936 = (struct std___Vector_base_int__std__allocator_int_____Vector_impl_data*)((char *)&(t931->_M_impl) + 0);
    int* t937 = base936->_M_start;
    long diff938 = t935 - t937;
    unsigned long cast939 = (unsigned long)diff938;
    std___Vector_base_int__std__allocator_int______M_deallocate(t931, t933, cast939);
  {
    std___Vector_base_int__std__allocator_int______Vector_impl____Vector_impl(&t931->_M_impl);
  }
  return;
}

// function: _ZNSt15__new_allocatorIiEC2ERKS0_
void std____new_allocator_int_____new_allocator(struct std____new_allocator_int_* v940, struct std____new_allocator_int_* v941) {
bb942: ;
  struct std____new_allocator_int_* this943;
  struct std____new_allocator_int_* unnamed944;
  this943 = v940;
  unnamed944 = v941;
  struct std____new_allocator_int_* t945 = this943;
  return;
}

// function: _ZNSt12_Vector_baseIiSaIiEE17_Vector_impl_dataC2Ev
void std___Vector_base_int__std__allocator_int______Vector_impl_data___Vector_impl_data(struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* v946) {
bb947: ;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* this948;
  this948 = v946;
  struct std___Vector_base_int__std__allocator_int_____Vector_impl_data* t949 = this948;
  int* c950 = ((void*)0);
  t949->_M_start = c950;
  int* c951 = ((void*)0);
  t949->_M_finish = c951;
  int* c952 = ((void*)0);
  t949->_M_end_of_storage = c952;
  return;
}

// function: _ZSt10destroy_atIiEvPT_
void void_std__destroy_at_int_(int* v953) {
bb954: ;
  int* __location955;
  __location955 = v953;
  return;
}

// function: _ZSt8_DestroyIPiEvT_S1_
void void_std___Destroy_int__(int* v956, int* v957) {
bb958: ;
  int* __first959;
  int* __last960;
  __first959 = v956;
  __last960 = v957;
      _Bool r961 = std____is_constant_evaluated();
      if (r961) {
          while (1) {
            int* t963 = __first959;
            int* t964 = __last960;
            _Bool c965 = ((t963 != t964)) ? 1 : 0;
            if (!c965) break;
            int* t966 = __first959;
            void_std__destroy_at_int_(t966);
          for_step962: ;
            int* t967 = __first959;
            int c968 = 1;
            int* ptr969 = &(t967)[c968];
            __first959 = ptr969;
          }
      }
  return;
}

// function: _ZSt8_DestroyIPiiEvT_S1_RSaIT0_E
void void_std___Destroy_int___int_(int* v970, int* v971, struct std__allocator_int_* v972) {
bb973: ;
  int* __first974;
  int* __last975;
  struct std__allocator_int_* unnamed976;
  __first974 = v970;
  __last975 = v971;
  unnamed976 = v972;
  int* t977 = __first974;
  int* t978 = __last975;
  void_std___Destroy_int__(t977, t978);
  return;
}

