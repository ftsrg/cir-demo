...
bool Mapper::mapModule(ModuleOp module, std::ostream &out) {
  ...
  // Emit a one-time preamble with only the headers needed by observed
  // external-library usage in this module.
  static bool headersEmitted = false;
  if (!headersEmitted) {
    std::set<std::string> definedSymbols;
    for (auto &op : module.getOps()) {
      if (op.getName().getStringRef() == "cir.func") {
        if (auto sym = op.getAttrOfType<StringAttr>(mlir::SymbolTable::getSymbolAttrName())) {
          definedSymbols.insert(sym.getValue().str());
        }
      }
    }

    std::set<std::string> calledSymbols;
    bool hasVarArgsOps = false;
    module.walk([&](mlir::Operation *op) {
      llvm::StringRef opName = op->getName().getStringRef();
      if (opName == "cir.va_start" || opName == "cir.va_end" ||
          opName == "cir.va_arg" || opName == "cir.va_copy") {
        hasVarArgsOps = true;
      }
      if (opName != "cir.call") return;
      if (auto sa = op->getAttrOfType<StringAttr>("callee")) {
        calledSymbols.insert(sa.getValue().str());
      } else if (auto sa2 = op->getAttrOfType<SymbolRefAttr>("callee")) {
        calledSymbols.insert(sa2.getRootReference().str());
      }
    });

    std::set<std::string> externalCalls;
    for (const auto &sym : calledSymbols) {
      if (!definedSymbols.count(sym)) externalCalls.insert(sym);
    }

    auto needsAny = [&](std::initializer_list<const char *> names) {
      for (const char *name : names) {
        if (externalCalls.count(name)) return true;
      }
      return false;
    };

    // Keep order stable and conventional.
    if (needsAny({"strlen", "strcmp", "strerror", "strcpy", "strchr", "memset", "memcmp",
                  "memcpy", "memmove", "strncmp", "strrchr", "strstr", "strncpy", "strtok",
                  "strspn", "strcspn"})) {
      out << "#include <string.h>\n";
    }
    if (needsAny({"malloc", "free", "realloc", "abort", "exit", "getenv", "qsort", "atoi",
                  "strtol", "strtoll", "strtoul", "strtoull", "strtod", "vasprintf"})) {
      out << "#include <stdlib.h>\n";
    }
    if (needsAny({"printf", "fprintf", "sprintf", "sscanf", "puts", "putchar", "fopen", "fclose",
                  "fread", "fwrite", "fseek", "fflush", "ferror", "fgetc", "fputs", "clearerr",
                  "vsnprintf"})) {
      out << "#include <stdio.h>\n";
    }
    if (hasVarArgsOps || needsAny({"vasprintf", "vsnprintf"})) {
      out << "#include <stdarg.h>\n";
    }
    if (needsAny({"time", "localtime", "mktime", "strftime", "ctime"})) {
      out << "#include <time.h>\n";
    }
    if (needsAny({"setjmp", "longjmp"})) {
      out << "#include <setjmp.h>\n";
    }
    if (needsAny({"close", "read", "write", "lseek", "chdir", "rmdir", "unlink", "sleep", "usleep"})) {
      out << "#include <unistd.h>\n";
    }

    if (!externalCalls.empty() || hasVarArgsOps) {
      out << "\n";
    }
    headersEmitted = true;
  }
