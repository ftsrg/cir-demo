...
usage() {
  echo "Usage: $0 [--flatten] <input-file> <output-dir>" >&2
  exit 1
}
...
if [[ "$FLATTEN" == "true" ]]; then
  # Flat mode: flatten with cir-opt before running xcfa-mapper.
  ...
  # Default non-flat mode: run xcfa-mapper directly on the structured CIR.
